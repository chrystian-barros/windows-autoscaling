...
resource "oci_vault_secret" "datadog_api_key" {
  ...
  secret_name = "${var.project.environment}-${var.project.prefix}-datadog_api_key-${formatdate("YYYY-MM-DD", timestamp())}"
  description = "API Key for Datadog agent authentication and sync"

  # Conteúdo do secret (em Bundle) — use BASE64 quando enviar conteúdo bruto
  secret_content {
    content_type = "BASE64"
    content      = base64encode(var.secret.datadog_api_key)
    name         = "1"       # nome da versão/conteúdo; opcional
    stage        = "CURRENT" # estágio; opcional
  }
...
resource "oci_vault_secret" "jenkins_api_key" {
  ...
  secret_name = "${var.project.environment}-${var.project.prefix}-jenkins_api_key-${formatdate("YYYY-MM-DD-HH", timestamp())}"
  description = "${var.project.prefix} - API Key for Jenkins service account authentication"

  # Conteúdo do secret (em Bundle) — use BASE64 quando enviar conteúdo bruto
  secret_content {
    content_type = "BASE64"
    content      = base64encode(var.secret.jenkins_api_key)
    name         = "1"       # nome da versão/conteúdo; opcional
    stage        = "CURRENT" # estágio; opcional
  }
